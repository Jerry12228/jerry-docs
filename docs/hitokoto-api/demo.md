...
* <https://hitokoto-api.jerry.ink/> （从7种分类中随机抽取）
* <https://hitokoto-api.jerry.ink/?c=b> （请求获得一个分类是漫画的句子）
* <https://hitokoto-api.jerry.ink/?c=f&encode=text> （请求获得一个来自网络的句子，并以纯文本格式输出）
...
<script src="https://hitokoto-api.jerry.ink/?encode=js&select=%23hitokoto" defer></script>
```
...
fetch('https://hitokoto-api.jerry.ink/')
  .then(response => response.json())
  .then(data => {
    const hitokoto = document.querySelector('#hitokoto_text')
    hitokoto.href = `https://hitokoto-api.jerry.ink/?uuid=${data.uuid}`
...
```js [Axios]
axios.get('https://hitokoto-api.jerry.ink/')
  .then(({ data }) => {
    const hitokoto = document.querySelector('#hitokoto_text')
    hitokoto.href = `https://hitokoto-api.jerry.ink/?uuid=${data.uuid}`
    hitokoto.innerText = data.hitokoto
...
$.ajax({
  type: 'GET',
  url: 'https://hitokoto-api.jerry.ink/',
  dataType: 'jsonp',
  jsonp: 'callback',
  jsonpCallback: 'hitokoto',
  success (data) {
    $('#hitokoto_text').attr('href', `https://hitokoto-api.jerry.ink/?uuid=${data.uuid}`)
...
xhr.open('get', 'https://hitokoto-api.jerry.ink/');
...
async function fetchHitokoto() {
  const response = await fetch('https://hitokoto-api.jerry.ink/')
  const { uuid, hitokoto: hitokotoText } = await response.json()
  const hitokoto = document.querySelector('#hitokoto_text')
  hitokoto.href = `https://hitokoto-api.jerry.ink/?uuid=${uuid}`
...
function fetch163Playlist(playlistId) {
  return new Promise((ok, err) => {
    fetch(`https://hitokoto-api.jerry.ink/nm/playlist/${playlistId}`)
      .then(response => response.json())
      .then(data => {
        const arr = [];
...
function fetch163Songs(Ids) {
  return new Promise(function (ok, err) {
    ...
    fetch(`https://hitokoto-api.jerry.ink/nm/summary/${ids.join(',')}?lyric=true&common=true`)
      .then(response => response.json())
      .then(data => {
        var songs = [];
